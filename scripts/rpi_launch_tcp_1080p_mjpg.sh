...
#gst-launch-1.0 -v v4l2src device=/dev/video0 io-mode=4 ! image/jpeg,width=1920,height=1080,type=video,framerate=30/1 ! rtpjpegpay ! queue ! udpsink host=$HOST port=5602 > /dev/null &
gst-launch-1.0 \
        videotestsrc \
        ! videoconvert ! videoscale ! video/x-raw,width=1920,height=1080 \
        ! clockoverlay shaded-background=true font-desc="Sans 38" \
        ! theoraenc ! oggmux ! tcpserversink host=localhost port=8081 > /dev/null &
