...
cd ..
ssh $ssh_arg $rpi_user@$rpi_ip "cd ~;mkdir -p $rpi_modbus_src_dir; mkdir -p $rpi_modbus_src_dir/jsmn;" 

# Send sources
scp $scp_arg $local_sources_modbus_dir/main.c \
...
scp $scp_arg $local_sources_modbus_dir/jsmn/jsmn.h \
        $rpi_user@$rpi_ip:$rpi_modbus_src_dir/jsmn

# Send Makefile
scp $scp_arg $local_sources_dir/Makefile \
